...
import { getDeSoData, getNFTdetails, getNFTEntries, getPostDetails, getUserStateless } from '../../lib/deso-controller'
    ...
    switch (paymentType) {
      case Enums.values.CREATOR:
        switch (creatorCoinPaymentType) {
          case 'default':
            creatorCoinBalance = desoData.profile.ccBalance / Enums.values.NANO_VALUE
            break
          default:
            creatorCoinBalance =
              desoData.profile.ccHodlers[
                desoData.profile.ccHodlers.findIndex(
      ...
      case Enums.values.DAO:
        switch (daoCoinPaymentType) {
          case 'default':
            daoBalance = desoData.profile.daoBalance
            break
          default:
            daoBalance =
              hexToInt(
                desoData.profile.daoHodlers[
                  desoData.profile.daoHodlers.findIndex(
      ...
      <>
        <span style={{ fontSize: 15 }}>
          {daoCoinPaymentType === 'default' ? (
            <b>DAO Balance: {desoData.profile.daoBalance}</b>
          ) : (
            <b>
              DAO Balance:{' '}
              {(
      ...
      <>
        <span style={{ fontSize: 15 }}>
          {creatorCoinPaymentType === 'default' ? (
            <b>Creator Coin Balance: {desoData.profile.ccBalance / Enums.values.NANO_VALUE}</b>
          ) : (
            <b>
